...
function setupRealtimeCases(teamId) {
  ...
  unsubscribe = listenToTeamDST(teamId, (cases) => {
     ...
     const incoming = cases.map(c => {
       ...
       const getMaterialOrderSummary = () => {
   
         const values = [
           ...new Set(
             rows
               .map(r => (r.materialOrder || "").trim())
               .filter(Boolean)
           )
         ];
       ...
       const getEtaSummary = () => {
         ...
         const etas = [
           ...new Set(
             rows
               .map(r => getEtaFromNotes(r.notesHistory || ""))
               .filter(Boolean)
           )
         ];
   
         if (etas.length === 0) return "";
         if (etas.length === 1) return etas[0];
       ...
       return {
             ...
             hasMultipleRows: !!c.hasMultipleRows,
       ...
       block.innerHTML = `
         <div class="filter-head" data-key="${key}">
           ...
           <div style="display:flex;align-items:center;gap:.5rem;">
             ...
             ${key === "country" ? `
               <div 
                 class="exclude-toggle ${uiState.countryInvert ? "active" : ""}" 
         ...
         if (key === "country") {
           ...
           if (toggle) {
              toggle.onclick = (e) => {
               ...
               // UI update
               toggle.classList.toggle("active", uiState.countryInvert);
   ...
   if (uiState.statusList.includes("NOT_ACTIONED_TODAY")) {
     ...
     rows = rows.filter(r => {
       if (!r.statusChangedOn) return false;
       return r.statusChangedOn !== today;
     });
   }
